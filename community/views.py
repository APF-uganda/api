...
class ForumPostViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """
        Set the author to the current user when creating a post and send email notifications
        """
        try:
            post = serializer.save(author=self.request.user)
            print(f" Post created successfully: {post.title} by {post.author.email}")
            
            # Send email notification to all members asynchronously
            from threading import Thread
            
            def send_notifications():
                try:
                    from .email_service import send_new_post_notification
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Filter comments by post if provided - optimized with prefetch
        """
        ...
        if post_id:
            ...
            # Prefetch replies with their authors for better performance
    ...
    @swagger_auto_schema(
        operation_description="Create a comment or reply. Set 'parent' to reply to another comment (supports unlimited nesting).",
        request_body=CommentSerializer,
        responses={201: CommentSerializer(), 400: 'Bad Request', 401: 'Unauthorized'},
        tags=['Forum - Comments']
    )
    def create(self, request, *args, **kwargs):
        return super().create(request, *args, **kwargs)
    ...
    def perform_create(self, serializer):
        """
        Set the author to the current user when creating a comment and send email notifications
        """
        ...
        def send_notifications():
            try:
                from .email_service import send_new_comment_notification, send_post_reply_notification
                # Notify post author
                send_post_reply_notification(comment)
                # Notify other participants in the discussion
